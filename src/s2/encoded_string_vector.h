...
class EncodedStringVector {
 public:
  ...
  s2::abslstring_view operator[](int i) const;
  ...
  // no longer needed.
  std::vector<s2::abslstring_view> Decode() const;
...
inline s2::abslstring_view EncodedStringVector::operator[](int i) const {
  uint64 start = (i == 0) ? 0 : offsets_[i - 1];
  uint64 limit = offsets_[i];
  return s2::abslstring_view(data_ + start, limit - start);
}
