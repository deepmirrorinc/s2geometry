...
S2CellId S2CellId::FromFaceIJ(int face, int i, int j) {
  ...
  // of the function.
  uint64 n = s2::abslimplicit_cast<uint64>(face) << (kPosBits - 1);
...
string S2CellId::ToString() const {
  if (!is_valid()) {
    return StrCat("Invalid: ", s2::abslHex(id(), s2::abslkZeroPad16));
  }
...
S2CellId S2CellId::FromDebugString(s2::abslstring_view str) {
