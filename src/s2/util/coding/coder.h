...
inline void DecoderExtensions::FillArray(Decoder* array, int num_decoders) {
  ...
  static_assert(s2::abslis_trivially_copy_constructible<Decoder>::value,
                "Decoder must be trivially copy-constructible");
  static_assert(s2::abslis_trivially_copy_assignable<Decoder>::value,
                "Decoder must be trivially copy-assignable");
  static_assert(s2::abslis_trivially_destructible<Decoder>::value,
...
inline void Encoder::putfloat(float f) {
  put32(s2::abslbit_cast<uint32>(f));
}

inline void Encoder::putdouble(double d) {
  put64(s2::abslbit_cast<uint64>(d));
}
...
inline float Decoder::getfloat() {
  return s2::abslbit_cast<float>(get32());
}

inline double Decoder::getdouble() {
  return s2::abslbit_cast<double>(get64());
}
