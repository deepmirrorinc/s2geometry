...
struct is_comparator_transparent<Compare,
                                 s2::abslvoid_t<typename Compare::is_transparent>>
    : std::true_type {};
...
template <>
struct key_compare_to_adapter<std::less<s2::abslstring_view>> {
  struct type : public btree_key_compare_to_tag {
    type() = default;
    explicit type(const std::less<s2::abslstring_view> &) {}
    int operator()(const s2::abslstring_view a, const s2::abslstring_view b) const {
      return a.compare(b);
    }
  };
};
...
template <>
struct key_compare_to_adapter<std::greater<s2::abslstring_view>> {
  struct type : public btree_key_compare_to_tag {
    type() = default;
    explicit type(const std::greater<s2::abslstring_view> &) {}
    int operator()(const s2::abslstring_view a, const s2::abslstring_view b) const {
      return b.compare(a);
    }
  };
...
struct has_linear_node_search_preference<
    T, s2::abslvoid_t<typename T::goog_btree_prefer_linear_node_search>>
    : std::true_type {};
template <typename T>
...
struct common_params {
  ...
  using key_compare =
      s2::abslconditional_t<btree_is_key_compare_to<Compare>::value, Compare,
  ...
  using node_count_type =
      s2::abslconditional_t<(kNodeValueSpace / ValueSize >
...
struct map_params : common_params<Key, Compare, Alloc, TargetNodeSize,
  ...
  using slot_type = s2::abslcontainer_internal::slot_type<Key, mapped_type>;
  ...
  struct value_compare : private key_compare {
    ...
    s2::abslconditional_t<btree_is_key_compare_to<key_compare>::value, int, bool>
...
struct set_params
  ...
  // s2::abslcontainer_internal::slot_type interface.
  struct slot_type {
    value_type value;

    template <class... Args>
    static void construct(Alloc *alloc, slot_type *slot, Args &&... args) {
      s2::abslallocator_traits<Alloc>::construct(*alloc, &slot->value,
    ...
    static void destroy(Alloc *alloc, slot_type *slot) {
      s2::abslallocator_traits<Alloc>::destroy(*alloc, &slot->value);
    }
...
class btree_node {
 ...
 public:
  ...
  using use_linear_search = s2::abslconditional_t<
 ...
 public:
  ...
  void clear_child(int i) {
    s2::abslcontainer_internal::SanitizerPoisonObject(&mutable_child(i));
  }
  void set_child(int i, btree_node *c) {
    s2::abslcontainer_internal::SanitizerUnpoisonObject(&mutable_child(i));
    mutable_child(i) = c;
    c->set_position(i);
  }
  ...
  static btree_node *init_leaf(btree_node *n, btree_node *parent,
    ...
    s2::abslcontainer_internal::SanitizerPoisonMemoryRegion(
 ...
 private:
  ...
  void value_init(const size_type i, allocator_type *alloc, Args &&... args) {
    s2::abslcontainer_internal::SanitizerUnpoisonObject(slot(i));
    slot_type::construct(alloc, slot(i), std::forward<Args>(args)...);
  }
  void value_destroy(const size_type i, allocator_type *alloc) {
    slot_type::destroy(alloc, slot(i));
    s2::abslcontainer_internal::SanitizerPoisonObject(slot(i));
  }
  ...
  void uninitialized_move_n(const size_type n, const size_type i,
                            ...
                            allocator_type *alloc) {
    s2::abslcontainer_internal::SanitizerUnpoisonMemoryRegion(
...
template <typename T>
using is_trivially_copyable = s2::abslconjunction<
    s2::abslis_trivially_copy_constructible<T>,
    s2::absldisjunction<s2::abslis_trivially_copy_assignable<T>,
                      s2::abslnegation<std::is_copy_assignable<T>>>,
    s2::abslis_trivially_destructible<T>>;
...
class btree {
  ...
  template <typename K>
  using const_lookup_key_reference = s2::abslconditional_t<
 ...
 public:
  ...
  btree(btree &&x) noexcept
      : root_(std::move(x.root_)),
        rightmost_(s2::abslexchange(x.rightmost_, nullptr)),
        size_(s2::abslexchange(x.size_, 0)) {
 ...
 private:
  ...
  node_type *allocate(const size_type size) {
    return reinterpret_cast<node_type *>(
        s2::abslcontainer_internal::Allocate<node_type::Alignment()>(
  ...
  void deallocate(const size_type size, node_type *node) {
    s2::abslcontainer_internal::Deallocate<node_type::Alignment()>(
 ...
 private:
  ...
  s2::abslcontainer_internal::CompressedTuple<key_compare, allocator_type,
                                            node_type *>
      root_;
  ...
  static_assert(std::is_same<s2::abslresult_of_t<key_compare(key_type, key_type)>,
                             s2::abslconditional_t<is_key_compare_to::value, int,
                                                 bool>>::value,
